...
A standalone, vitiate-only coverage/throughput benchmark for **node-forge** - a
pure-CommonJS recursive ASN.1/DER -> X.509/PKCS decoder. It exists to exercise
vitiate's `instrument.packages` CJS-instrumentation path on a deep real target.
No cloning or vendoring: it drives the local `@vitiate/core` build via
`workspace:*`.
...
# or: pnpm --filter @vitiate/bench-forge run bench
```
