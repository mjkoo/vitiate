...
8. If none of the above can start: return `null`.
...
#### Scenario: Stage skipped when no CmpLog data and Grimoire disabled and unicode disabled
...
#### Scenario: Unicode begins when I2S skipped and Grimoire not applicable
...
- **THEN** `beginStage()` SHALL compute `UnicodeIdentificationMetadata` for the corpus entry (if not cached)
- **AND** if valid UTF-8 regions exist, return a non-null `Buffer` containing the first unicode-mutated input
- **AND** `StageState` SHALL transition to `Unicode`

#### Scenario: Unicode begins when I2S skipped, Grimoire enabled but not applicable

- **WHEN** `reportResult()` returned `Interesting` and calibration has completed
- **AND** `CmpValuesMetadata` is empty
- **AND** Grimoire is enabled
- **AND** the corpus entry does NOT qualify for generalization AND has no pre-existing `GeneralizedInputMetadata`
- **AND** unicode is enabled
- **THEN** `beginStage()` SHALL compute `UnicodeIdentificationMetadata` for the corpus entry (if not cached)
- **AND** if valid UTF-8 regions exist, return a non-null `Buffer` containing the first unicode-mutated input
- **AND** `StageState` SHALL transition to `Unicode`
...
#### Scenario: Grimoire stage completes without unicode
...
- `Unicode { corpus_id: CorpusId, iteration: usize, max_iterations: usize }`: A unicode mutational stage is in progress.
...
- `None` → `Unicode`: Via `beginStage()` when I2S is skipped, Grimoire stages are not applicable (disabled, OR entry does not qualify for generalization and has no pre-existing `GeneralizedInputMetadata`), unicode is enabled, and the corpus entry has valid UTF-8 regions.
...
- `I2S` → `Unicode`: Via `advanceStage()` when I2S completes and Grimoire stages are not applicable (disabled, OR entry does not qualify for generalization and has no pre-existing `GeneralizedInputMetadata`), unicode is enabled, and the corpus entry has valid UTF-8 regions.
- `I2S` → `None`: Via `advanceStage()` when I2S completes and Grimoire stages are not applicable and unicode is not applicable.
...
- `Generalization` → `None`: Via `advanceStage()` when generalization fails (verification) or is abandoned. The unicode stage SHALL NOT be attempted after generalization failure (unstable inputs produce unreliable coverage).
- `Grimoire` → `Grimoire`: Via `advanceStage()` when `iteration < max_iterations`.
- `Grimoire` → `Unicode`: Via `advanceStage()` when iterations exhausted, unicode is enabled, and the corpus entry has valid UTF-8 regions.
- `Grimoire` → `None`: Via `advanceStage()` when iterations exhausted and unicode is not applicable (disabled, or no valid UTF-8 regions exist).
- `Unicode` → `Unicode`: Via `advanceStage()` when `iteration < max_iterations`.
...
#### Scenario: Full four-stage pipeline lifecycle
...
- **THEN** `StageState` transitions `None` → `I2S` → ... → `Generalization` → ... → `Grimoire` → ... → `Unicode` → ... → `None`

#### Scenario: I2S-to-unicode pipeline (Grimoire not applicable)
...
#### Scenario: Grimoire-to-unicode pipeline

- **WHEN** `beginStage()` starts an I2S stage
- **AND** I2S completes and Grimoire is enabled
- **AND** generalization succeeds and Grimoire completes
- **AND** unicode is enabled
- **THEN** `StageState` transitions `None` → `I2S` → ... → `Generalization` → ... → `Grimoire` → ... → `Unicode` → ... → `None`

#### Scenario: I2S-to-Grimoire pipeline (pre-existing metadata, unicode disabled)
...
#### Scenario: I2S-only pipeline (Grimoire and unicode disabled)
...
- **AND** generalization succeeds
- **THEN** `StageState` transitions `None` → `Generalization` → ... → `Grimoire` → ... (→ `Unicode` if enabled →) `None`
...
#### Scenario: All stage types counted including unicode

- **WHEN** a pipeline runs I2S (64 iterations) + generalization (20 executions) + Grimoire (32 iterations) + unicode (48 iterations)
- **THEN** `total_execs` SHALL increment by 164 for the stage pipeline
