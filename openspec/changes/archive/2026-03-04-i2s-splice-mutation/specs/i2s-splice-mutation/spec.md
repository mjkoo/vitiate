...
#### Scenario: Splice replaces shorter match with longer operand
...
- **AND** the coin flip selects splice
...
#### Scenario: Overwrite truncates replacement to matched prefix length

- **WHEN** `CmpValuesMetadata` contains `CmpValues::Bytes("http", "javascript")`
- **AND** the input contains `"http://example.com"` (full 4-byte match, so `matched_prefix_len` = 4)
- **AND** the coin flip selects overwrite
- **THEN** the mutator SHALL write the first 4 bytes of `"javascript"` over the match position
- **AND** the mutated input SHALL be `"java://example.com"` (18 bytes, unchanged length)
...
- **THEN** the mutator SHALL fall back to overwrite
- **AND** the input length SHALL remain 120 bytes

### Requirement: Bidirectional operand matching

The mutator SHALL scan for both operands of a `CmpValues::Bytes` entry in the input, matching in both directions: if `v.0` is found, replace with `v.1`; if `v.1` is found, replace with `v.0`. The first match found (scanning from a random offset) SHALL be used. The splice-vs-overwrite coin flip applies regardless of which direction matched.
...
#### Scenario: Partial match with splice
...
#### Scenario: Partial match with overwrite

- **WHEN** `CmpValuesMetadata` contains `CmpValues::Bytes("http", "javascript")`
- **AND** the input contains `"htt"` (3-byte prefix match, `matched_prefix_len` = 3) but not the full `"http"`
- **AND** the coin flip selects overwrite
- **THEN** the mutator SHALL overwrite the 3 matched bytes with `"jav"` (first 3 bytes of `"javascript"`)
- **AND** the input length SHALL remain unchanged
