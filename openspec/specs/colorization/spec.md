...
Colorization identifies free byte ranges in a corpus entry that do not affect the coverage pattern, enabling REDQUEEN to make targeted replacements only in regions that are safe to modify. This capability defines the binary search algorithm, type-preserving byte replacement, coverage hashing, taint metadata storage, and the dual CmpLog trace that captures comparison values from the colorized input.
