...
Grimoire auto-detection determines whether to enable Grimoire (generalization + Grimoire mutation stages) based on corpus content analysis. It supports explicit override via configuration, deferred detection for empty corpora, and exposes the detection result to the stage pipeline.
