...
The Grimoire mutational stage generates structure-aware mutations using `GeneralizedInputMetadata` produced by the generalization stage. It uses a `HavocScheduledMutator` wrapping five Grimoire-specific mutators to produce inputs that respect the structural patterns discovered during generalization.
