...
REDQUEEN mutation generates transform-aware targeted replacement candidates using LibAFL's `AflppRedQueen::multi_mutate()`. This capability defines the REDQUEEN stage within the stage pipeline, including candidate generation, stage state, metadata requirements, and the interaction with the colorization stage that precedes it.
