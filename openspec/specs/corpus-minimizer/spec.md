...
Corpus minimization tracks the best (smallest and fastest) corpus entry for each observed coverage edge and uses favored status to bias scheduling toward the most efficient entries. This reduces corpus bloat and focuses fuzzing effort on inputs that cover unique edges with minimal overhead.
...
- **AND** corpus entry B displaces A for edge 10 (B has lower penalty)
- **THEN** A SHALL NOT own any edge in `TopRatedsMetadata`
- **AND** B SHALL own edge 10 in `TopRatedsMetadata`
- **AND** after `cull` is called, B SHALL have `IsFavoredMetadata`
- **AND** A MAY still have `IsFavoredMetadata` (stale marker from prior `cull`; not removed by LibAFL)
