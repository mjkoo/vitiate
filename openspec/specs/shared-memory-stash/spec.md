...
The shared-memory-stash capability provides a cross-process shared memory region for stashing fuzz inputs. It enables the parent supervisor and watchdog to recover crashing/timeout inputs even when the child process dies unexpectedly. Replaces the previous in-process InputStash.
