...
- `VITIATE_FUZZ_TIME` SHALL take precedence over `VITIATE_OPTIONS.fuzzTimeMs`. An invalid `VITIATE_FUZZ_TIME` SHALL NOT clobber a valid `VITIATE_OPTIONS.fuzzTimeMs`.

Precedence (highest wins): `VITIATE_FUZZ_TIME` > `VITIATE_OPTIONS.fuzzTimeMs` > per-test `fuzzTimeMs` option.
...
- **AND** this is equivalent to `VITIATE_OPTIONS={"fuzzTimeMs":30000}`

#### Scenario: VITIATE_FUZZ_TIME overrides VITIATE_OPTIONS

- **WHEN** `VITIATE_FUZZ_TIME=10` and `VITIATE_OPTIONS={"fuzzTimeMs":60000}` are both set
