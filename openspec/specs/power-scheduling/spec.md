...
Power-aware corpus scheduling weights corpus entry selection based on execution speed, coverage contribution, mutation depth, recency, and fuzz count. Combined with calibration and unstable edge masking, this focuses fuzzing effort on the most productive entries while filtering out non-deterministic coverage noise.
...
- **THEN** the fuzzer's unstable entries set SHALL contain {42, 100, 200}
...
If the fuzzer's unstable entries set is empty (no calibration has completed yet or no unstable edges found), masking SHALL be skipped and all coverage map entries SHALL be evaluated normally.
