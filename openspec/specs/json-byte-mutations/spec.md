...
JSON byte mutations provide structure-aware mutation operators that operate directly on JSON byte buffers without parsing into a DOM. These mutators replace string values, object keys, and arbitrary JSON values using dictionary tokens and type-changing strategies, enabling the fuzzer to make semantically meaningful changes to JSON inputs while maintaining structural validity.
