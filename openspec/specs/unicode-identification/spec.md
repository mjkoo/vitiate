...
Unicode identification analyzes byte inputs to extract valid UTF-8 string regions with precomputed character boundaries. This metadata enables unicode mutators to operate on character-level structures within potentially mixed binary/text inputs. The identification result is cached per corpus entry to avoid redundant computation.
